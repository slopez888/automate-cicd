...
# Updated by Eric Lavarde, Pull Request on-going https://github.com/ansible/ansible/pull/33282
# Name changed to avoid issue https://github.com/ansible/ansible/issues/33484
...
class CallbackModule(CallbackBase):
    ...
    def _build_test_case(self, task_data, host_data):
        ...
        if self._task_class == 'true':
            matchobj = re.match(r'^(.*)\.yml:([0-9]+)$', junit_classname)
            if matchobj:
                junit_classname = matchobj.group(1)
                name = '[%s] %s <line %s>: %s' % (
                    host_data.name, task_data.play, matchobj.group(2), task_data.name)
